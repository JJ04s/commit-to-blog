...
* **AI 요약 기능 (Custom Skills)**: 선택한 커밋의 변경 사항(Diff) 데이터를 Gemini API로 전송하여 포스트 초안(제목, 본문) 자동 생성. 단순 요약이 아닌, 사용자의 취향(예: 논리적 전개 방식, 배경 지식 포함 등)이 반영된 전용 '스킬' 로직을 적용하여 고품질의 초안을 생성함.
...
* **Gemini 연동**: `@google/genai` 등을 활용해 커밋 Diff 데이터와 프롬프트를 전송하고 JSON 형태로 응답 수신. 단순 프롬프트가 아닌, '배경 지식 -> 작업 내용 -> 결론'으로 이어지는 논리적 연결고리를 강조하는 커스텀 요약 로직(스킬)을 구축하여 적용함.
